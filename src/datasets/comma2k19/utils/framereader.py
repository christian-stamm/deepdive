...
from .cache import cache_path_for_file_path
from .exceptions import DataUnreadableError
from .file_helpers import atomic_write_in_dir
...
def vidindex(fn, typ):
    ...
    with tempfile.NamedTemporaryFile() as prefix_f, tempfile.NamedTemporaryFile() as index_f:
        try:
...
def cache_fn(func):
    ...
    def cache_inner(fn, *args, **kwargs):
        ...
        if cache_path and os.path.exists(cache_path):
            ...
            if cache_path:
                with atomic_write_in_dir(
                    cache_path, mode="wb", overwrite=True
                ) as cache_file:
...
def FrameReader(
    fn, cache_prefix=None, readahead=False, readbehind=False, index_data=None
):
    ...
    if frame_type == FrameType.raw:
        ...
        return StreamFrameReader(
            fn, frame_type, index_data, readahead=readahead, readbehind=readbehind
        )
    else:
...
class StreamGOPReader(GOPReader):
    ...
    def _lookup_gop(self, num):
        ...
        frame_e = num + 1
        while (
            frame_e < (len(self.index) - 1) and self.index[frame_e, 0] != HEVC_SLICE_I
        ):
            frame_e += 1
...
def GOPFrameIterator(gop_reader, pix_fmt):
    ...
    IN_FLIGHT_GOPS = (
        6  # should be enough that the stream decompressor starts returning data
    )

    with VideoStreamDecompressor(
        gop_reader.vid_fmt, gop_reader.w, gop_reader.h, pix_fmt
    ) as dec:
        read_work = []
